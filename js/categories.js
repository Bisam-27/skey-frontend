    ...
    if (currentCategorySlug) {
      ...
      if (currentCategory && currentCategory.subcategories && currentCategory.subcategories.length > 0) {
        ...
        currentCategory.subcategories.forEach(subcategory => {
          const isActive = subcategory.slug === currentSubcategorySlug ? 'sidebar__list-item--active' : '';
          sidebarHTML += `
            <div class="sidebar__list-item ${isActive}" onclick="categoryManager.filterBySubcategory('${subcategory.slug}')">
              ${subcategory.name}
            </div>
          `;
        });
