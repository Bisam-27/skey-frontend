...
class CartManager {
  ...
  updatePaymentDetails(paymentDetails) {
    // Update bag total
    const bagTotalElement = document.querySelector('.cart__bag-total');
    if (bagTotalElement) {
      bagTotalElement.textContent = `₹${paymentDetails.bag_total || 0}`;
    }

    // Update bag discount
    const bagDiscountElement = document.querySelector('.cart__bag-discount');
    if (bagDiscountElement) {
      bagDiscountElement.textContent = `₹${paymentDetails.bag_discount || 0}`;
    }

    // Update delivery fee
    const deliveryFeeElement = document.querySelector('.cart__delivery-fee');
    if (deliveryFeeElement) {
      deliveryFeeElement.textContent = `₹${paymentDetails.delivery_fee || 0}`;
    }

    // Update amount payable
    const amountPayableElement = document.querySelector('.cart__amount-payable');
    if (amountPayableElement) {
      amountPayableElement.textContent = `₹${paymentDetails.amount_payable || paymentDetails.bag_total || 0}`;
    }

    // Show applied coupon info if any
    this.updateCouponDisplay(paymentDetails.applied_coupon);
  }

  // Update coupon display
  updateCouponDisplay(appliedCoupon) {
    const couponInput = document.querySelector('.cart__input');
    ...
    if (appliedCoupon) {
      ...
      if (couponButton) {
        ...
        couponButton.classList.add('cart__btn--remove');
      }
    } else {
      // Reset coupon input
      if (couponInput) {
  ...
  setupEventListeners() {
    ...
    if (couponButton) {
      couponButton.addEventListener('click', async (e) => {
        e.preventDefault();
        await this.handleCouponAction();
      });
    }

    // Coupon input enter key handler
    const couponInput = document.querySelector('.cart__input');
    if (couponInput) {
  ...
  async handleCouponAction() {
    ...
    const isRemoveAction = couponButton.classList.contains('cart__btn--remove');
  ...
  async applyCoupon(code) {
    try {
      this.showLoading();
      const response = await apiService.applyCouponToCart(code);

      if (response.success) {
        this.cart = response.data;
        await this.renderCart(true);
        this.updateNavbarCartCount();
  ...
  async removeCoupon() {
    try {
      this.showLoading();
      const response = await apiService.removeCouponFromCart();

      if (response.success) {
        this.cart = response.data;
        await this.renderCart(true);
        this.updateNavbarCartCount();
        this.showNotification('Coupon removed successfully', 'success');
      } else {
        throw new Error(response.message);
      }
    } catch (error) {
  ...
  proceedToCheckout() {
    ...
    if (!this.cart.total_amount || this.cart.total_amount <= 0) {
